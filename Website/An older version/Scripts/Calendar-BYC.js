function run(){
	...
	var forward = [370, 0, "->", true]
	var backward = [370, 0, "<-", false]
	var clear = [280, 0, "Clear",true]
	...
	function setNumbers(){		
		...
		while (dayCounter <= maxDay){
			...
			ctx.fillText(numberDate[rowDay][colDay], segmentX * (colDay) + 5, segmentY * (rowDay + 1) + twoSegmentY);
		...
		while (rowDay != 6){
			...
			ctx.fillText(numberDate[rowDay][colDay], segmentX * (colDay) + 10, segmentY * (rowDay + 1) + twoSegmentY - 10);
			colDay++;
			if(colDay > 6){
				colDay = 0;
				rowDay++;
		...
		while(firstDay != -1){
			...
			ctx.font = "30px Arial";
			ctx.fillStyle = numColor
			ctx.fillText(numberDate[0][firstDay], segmentX * (firstDay), segmentY * (1) + twoSegmentY);			
			ctx.globalAlpha=0.5;
			ctx.fillStyle = lightCover
				...
				for (var j = 0; j < 7; j++){
					if ( 25 > (coordinates[i][j][0] - xy[0]) && 25 > (coordinates[i][j][1] - xy[1])&& -25 < (coordinates[i][j][0] - xy[0]) && -25 < (coordinates[i][j][1] - xy[1])){
						...
						if(coordinates[i][j][2]){}
							...
							ctx.fillStyle = numColor
							ctx.fillText(numberDate[i][j], segmentX * (j) + 10, segmentY * (i + 1) + twoSegmentY - 10)
				...
				for (var j = 0; j < 3; j++){
					if ( 0 > (hourCoordinates[i][j][0] - xy[0]) && 0 > (hourCoordinates[i][j][1] - xy[1])&& -50 < (hourCoordinates[i][j][0] - xy[0]) && -40 < (hourCoordinates[i][j][1] - xy[1])){
						if(typeof hourCoordinates[i][j][3] == 'undefined'){
							...
							ctx.globalAlpha=0.5;
							ctx.fillStyle = darkCover
							ctx.fillRect(hourCoordinates[i][j][0], hourCoordinates[i][j][1], 50,40)
