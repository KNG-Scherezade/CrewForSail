	...
	function setNumbers(){		
		...
		while (rowDay != 6){
			...
			if(dayCounter-1 < 10){
				...
				ctx.fillText(numberDate[rowDay][colDay], segmentX * (colDay) + 12, segmentY * (rowDay + 1) + twoSegmentY - 13);
				...
				for (var j = 0; j < 7; j++){
					if ( 25 > (coordinates[i][j][0] - xy[0]) && 25 > (coordinates[i][j][1] - xy[1])&& -25 < (coordinates[i][j][0] - xy[0]) && -25 < (coordinates[i][j][1] - xy[1])){
						...
						if(coordinates[i][j][2]){}
							...
							yearString = currentYear + "/" + currentMonth + "/" + dayOfMonth;
							...
							currentMonth = currentMonth + 1
							if(currentMonth < 10) currentMonth = "0" + currentMonth;
							yearString = currentYear + "/" + currentMonth  + "/" + numberDate[i][j]	
							displayOne = true;
							displayTime(currentHour)
							hourOfDay = currentHour%12;
							timeString = yearString + " "  + hourOfDay + " " + hourSwitch
